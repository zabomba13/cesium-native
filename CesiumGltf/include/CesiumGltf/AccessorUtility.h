...
struct IndicesForFaceFromAccessor {
  std::array<int64_t, 3> operator()(std::monostate) {
    ...
    if (primitiveMode == MeshPrimitive::Mode::TRIANGLE_FAN) {
      ...
      for (int64_t i = 0; i < 3; i++) {
        ...
        result[size_t(i)] = vertexIndex < vertexCount ? vertexIndex : -1;
      }
    }
  ...
  std::array<int64_t, 3> operator()(const AccessorView<T>& value) {
    ...
    if (primitiveMode == MeshPrimitive::Mode::TRIANGLE_FAN) {
      result[0] = value[0];
      result[1] = firstIndex < value.size() ? int64_t(value[firstIndex]) : -1;
      result[2] =
          firstIndex + 1 < value.size() ? int64_t(value[firstIndex + 1]) : -1;
    } else {
      for (int64_t i = 0; i < 3; i++) {
        int64_t index = firstIndex + i;
        result[size_t(i)] = index < value.size() ? int64_t(value[index]) : -1;
      }
    }
