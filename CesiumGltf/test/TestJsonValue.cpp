...
TEST_CASE(
  ...
  const auto int64Max = std::numeric_limits<int64_t>::max();
  const auto int64Min = std::numeric_limits<int64_t>::min();
  ...
  const auto uint64Min = std::numeric_limits<uint64_t>::min();
...
TEST_CASE("JsonValue::getSafeNumber() throws if narrowing conversion error "
  ...
  SECTION("2^64 - 1 cannot be converted back to a double") {
    auto value = JsonValue(std::numeric_limits<uint64_t>::max());
  ...
  SECTION("-2^64 cannot be converted back to a double") {
    ...
    auto value = JsonValue(std::numeric_limits<int64_t>::min() + 1);
    REQUIRE_THROWS(value.getSafeNumber<double>());
  }

  SECTION("1024.0 cannot be converted back to a uint8_t") {
    auto value = JsonValue(1024.0);
    REQUIRE_THROWS(value.getSafeNumber<uint8_t>());
  }

  SECTION("1.5 cannot be converted back to a uint16_t") {
    auto value = JsonValue(1.5);
    REQUIRE_THROWS(value.getSafeNumber<uint16_t>());
  }
}
...
TEST_CASE("JsonValue::getSafeNumberOrDefault() returns default if narrowing "
  ...
  SECTION("1024.0 cannot be converted back to a uint8_t") {
    auto value = JsonValue(1024.0);
    REQUIRE(value.getSafeNumberOrDefault<uint8_t>(255) == 255);
  }

  SECTION("1.5 cannot be converted back to a uint16_t") {
    auto value = JsonValue(1.5);
    REQUIRE(value.getSafeNumberOrDefault<uint16_t>(365) == 365);
  }
}
