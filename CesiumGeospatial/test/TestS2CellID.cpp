...
TEST_CASE("S2CellID") {
  ...
  SECTION("correctly converts token to cell ID") {
    ...
    CHECK(
        S2CellID::fromToken("2ef59bd352b93ac3").getID() == 3383782026967071427U
    );
  }
  ...
  SECTION("computeBoundingRectangle") {
    ...
    CHECK(Math::equalsEpsilon(equatorRect.getEast(), 0.0, 0.0, Math::Epsilon14)
    );
    CHECK(Math::equalsEpsilon(
    ...
    CHECK(Math::equalsEpsilon(equatorRect.getNorth(), 0.0, 0.0, Math::Epsilon14)
    );
