...
std::optional<I3dmContent> parseI3dmJson(
  ...
  if (featureTableJson.HasParseError()) {
    errors.emplaceError(fmt::format(
        ...
        "{}",
        static_cast<int>(featureTableJson.GetParseError()),
  ...
  if (!(parsedContent.position || parsedContent.positionQuantized)) {
  ...
  if (parsedContent.normalUpOct32p && !parsedContent.normalRightOct32p) {
    errors.emplaceError(
...
CesiumAsync::Future<ConvertedI3dm> convertI3dmContent(
  ...
  if (parsedContent.scaleNonUniform) {
