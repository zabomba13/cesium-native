...
TEST_CASE("Test converting gltf extras to skirt mesh metadata") {
  ...
  SECTION("Gltf Extras has correct format") {
    ...
    REQUIRE(
        Math::equalsEpsilon(skirtMeshMetadata.meshCenter.x, 1.0, Math::Epsilon7)
    );
    REQUIRE(
        Math::equalsEpsilon(skirtMeshMetadata.meshCenter.y, 2.0, Math::Epsilon7)
    );
    REQUIRE(
        Math::equalsEpsilon(skirtMeshMetadata.meshCenter.z, 3.0, Math::Epsilon7)
    );
    REQUIRE(Math::equalsEpsilon(
        skirtMeshMetadata.skirtWestHeight,
        12.4,
